...
getCitySuggestions(query: string): Observable<string[]> {
  ...
  return this.http.get<any>(requestUrl).pipe(
    map((data: any) => {                                //map trasforma ogni valore emesso da un ob in un nuovo valore
      if (data.results && data.results.length > 0) {
        return data.results.map((result: any) => result.formatted); //mappa tutto in questo array
      } else {
        return []; //se non esiste vuoto.
      }
    })
  );
}
